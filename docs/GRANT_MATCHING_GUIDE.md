# 📄 Funding & Discovery Engine: Grant Matching Workflows (Backend)
## Technical Implementation, Integration Map & API Documentation

> **Role**: Grant Matching Workflows (Backend)  
...
builds the **Eligibility-Matching Rules Engine** and backend APIs for evaluating whether a researcher, startup founder, or innovation manager is eligible for funding opportunities.
...
3. **Data Schema & Models Collaboration** (`backend/models.py`, `backend/schemas.py`):
   ...
   - Seeded initial database with funding opportunities from all 6 sources specified in Release.
...
## 🔄 Integration Map — How Integrates with All 7 Members

```mermaid
graph TD
    M3["Funding Data Ingestion"] -->|PostgreSQL Grant Records & Eligibility Criteria| M2["Grant Matching Workflows (OUR ROLE)"]
    M2 -->|Eligible Grant List & Criteria Breakdown| M1["Recommendation Engine"]
    M2 -->|GET /grants/eligible API & Filters| M7["Recommendations UI"]
    M1 -->|POST /recommendations/generate| M7
    M5["Research Data Pipeline"] -->|MongoDB Cleaned Papers| M4["Trend Analysis NLP"]
    M4 -->|GET /trends/topics & /trends/hotspots| M6["Trend Dashboards UI"]
    M6 <-->|Shared Component Library & Design Tokens| M7
    M8["QA & Integration"] <-->|Shared DB Config & Integration Tests| M2
```

### Detailed Member-by-Member Hand-off Contracts:

#### 1. Integration with **(Funding Data Ingestion)**
- **Receives from **: Ingested grant records from 6 sources (*Government Grants, Research Councils, Innovation Funds, Accelerators, Venture Programs, Int'l Agencies*) stored in PostgreSQL `funding_opportunities` and `funding_sources` tables.
- **Contract**: reads `research_domain`, `career_stage`, `eligible_geography`, `funding_type`, `grant_amount`, and `deadline` from s tables.

#### 2. Integration with **(Funding Recommendation Engine)**
- **Hands off to **: feeds the pre-filtered, eligibility-matched grant list (`is_eligible = True`, `overall_eligibility_score > 50`) into s recommendation scoring pipeline (`POST /recommendations/generate`).
- **Contract**: consumes s `EligibilityMatchResult` output to apply ranking algorithms (weighting past success rates and deadline urgency).

#### 3. Integration with **(Funding Recommendations UI & Full-Stack)**
- **Exposes to **: 
...
- **Contract**: calls s endpoints directly or via s aggregated payload.

#### 4. Integration with **(QA, Integration & Documentation)**
- **Hands off to **: Shared PostgreSQL database connection layer, FastAPI router definitions, and `backend/tests/test_grant_matching.py` test suite for end-to-end integration testing.
