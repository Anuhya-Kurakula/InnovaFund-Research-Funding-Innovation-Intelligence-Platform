...
Added by (Kesiya) for Patent Landscape Analysis. Uses the same shared innovafund_db database - no separate database needed.
...
Seeded from a 10,000-row Lens.org patent CSV via a local seed script ().

### Technology Intelligence tables (PostgreSQL)

Added by (Mayank) for the Technology Intelligence Engine, on the origin/mayank branch. Also uses shared innovafund_db - no separate database needed. This branch is currently the most stable working backend for Release - see QA_FINDINGS.md.
...
| maturity_score | Float | 0.0-100.0 - supplies the 15% "Technology Maturity" weight to s Innovation Scoring model |
...
Two separate patent tables now coexist on the shared DB: `patents` (from Release/2) and `patent_records` (Kesiya's Release Patent Landscape work). These were not designed together and likely need to be reconciled - flagged to the team, not yet resolved as of 2026-09-02.